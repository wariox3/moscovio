...
import { FilterCondition, FilterField, Operator } from './interface/filtro.interface';
import { OPERADORES_FILTRO } from './constant/operadores-filtro.constant';
import { FilterTransformerService } from './service/filter-transformer.service';
import { GeneralRepository } from '@app/core';
import { MultiSelectComponent } from '@tamerlantian/ui-components';
...
export class FiltroComponent implements OnInit, OnChanges, OnDestroy {
  ...
  onFieldChange(condition: FilterCondition, index: number): void {
    ...
    const selectedField = this.availableFields.find(field => field.name === condition.field);
  ...
  applyFilters(): void {
    ...
    const parametros = this._filterTransformerService.transformToApiParams(validFilters);
  ...
  getOperatorsForField(fieldName: string): Operator[] {
    const field = this.availableFields.find(field => field.name === fieldName);
    if (!field) return [];
  ...
  private _loadRelationOptions(fieldName: string, searchTerm: string = ''): void {
    ...
    this._generalRepository.get<any[]>(config.endpoint, queryParams).subscribe({
      next: response => {
      ...
      error: error => {
        console.error(`Error al cargar opciones de relación para ${fieldName}:`, error);
        this.relationOptions[fieldName] = [];
      },
      complete: () => {
  ...
  onRelationOptionSelected(condition: FilterCondition, option: RelationOption): void {
  ...
  onRelationOptionSelectedMultiple(condition: FilterCondition, option: RelationOption): void {
